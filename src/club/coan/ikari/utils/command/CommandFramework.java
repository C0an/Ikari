package club.coan.ikari.utils.command;
...
public class CommandFramework implements CommandExecutor {
	...
	public boolean handleCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
		for (int i = args.length; i >= 0; i--) {
			...
			for (int d = 0; d < args.length - subCommand; d++) {
				modArgs[d] = args[d + subCommand];
			}
			if (commandMap.containsKey(cmdLabel)) {
				...
				if (!command.permission().equals("") && !sender.hasPermission(command.permission())) {
					sender.sendMessage(Locale.NOPERMISSION.toString());
					return true;
				}
				if (command.inGameOnly() && !(sender instanceof Player)) {
					sender.sendMessage(Locale.INGAMEONLY.toString());
					return true;
				}
				if (command.isConsoleOnly() && (sender instanceof Player)) {
				    sender.sendMessage(Locale.INCONSOLEONLY.toString());
				    return true;
                }
				...
				if (command.isAsync()) {
					...
					}.runTaskAsynchronously(Ikari.getInstance());
				} else {
					try {
						method.invoke(methodObject, new CommandArgs(sender, cmd, label, args,
								cmdLabel.split("\\.").length - 1));
	...
	private void defaultCommand(CommandArgs args) {
		args.getSender().sendMessage(Locale.INCORRECTSETUP.toString().replaceAll("%command%", args.getLabel()));
	}
